...
begin
  ...
  ShowMessage('incorrect input in getspecsamples64'+' '+IntToStr(startSample)+' '+IntToStr(amountToRead));
  //getTimestamp here...
...
begin
  if ((startSample>2047) or (startSample<0) or (amountToRead>2048)) then
  ShowMessage('incorrect input in getspecsamples64ns'+' '+IntToStr(startSample)+' '+IntToStr(amountToRead));
...
digital_delay :=1;
  ...
  begin;
    ...
    if (i=1) then
    begin
      prev := qElizabeth[0].counter;
      digital_delay := qElizabeth[0].counter;
      Continue;
    end;
    digital_delay:=qElizabeth[i-1].counter-prev;
    if(i>3) then
    begin
      if ((digital_delay>=0) and (digital_delay<4000)) then
      Inc(hist_digital[digital_delay])
        else
      inc(hist_digital[4000]);
    end;
    prev:= qElizabeth[i-1].counter;
  end;
end;
